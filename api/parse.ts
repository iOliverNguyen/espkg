...
import {URL} from "url";
import type {PackageInfo, RegistryPackageMeta} from './types.js';
...
// url must start with /
export function parsePath(input): PackageInfo {
...
export function findVersion(meta: RegistryPackageMeta, tag: string): string {
  // valid version
  if (semver.valid(tag)) return meta.versions[tag] && tag;

  // dist tag
  const distTags = meta['dist-tags']
  if (tag in distTags) return distTags[tag];
